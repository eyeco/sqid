...
#pragma pack(pop)
	...
	private:
		...
		std::map<MsgFlags, Compressor*> _comps;
#endif

		ComMsg _comMsg;

		size_t _reservedBytes;

		std::vector<unsigned char> _payloadBuffer;
		
#ifdef __COMPRESSION_SUPPORT
		std::map<MsgFlags, Compressor*> _comps;

		Compressor* getOrCreateEncoder( MsgFlags encoding );
#endif

		void reserve( size_t bytes );

		bool getPayload( MsgFlags flags, const SampleFrame* frame, bool normalize, bool clamp );

	public:
		explicit ComMsgBuilder();
		~ComMsgBuilder();

		const ComMsg *build( const SampleFrame* frame, unsigned char deviceID, unsigned char sensorID, MsgFlags flags, bool clamp );
	};
