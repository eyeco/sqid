...
namespace sqid
{
	namespace General
	{
		...
		//DEFINE_OP_DESC( ClickWheel, "clickWheel", "/gestures/deprecated",
		//	"4B2A3E86-9707-4229-83D7-C17DB66ECD77" );
		...
		/*
		ClickWheel::ClickWheel() : // float scale, float offset ) :
			Op(),
			_blobThresh( 0.1f ),
			_currentPos( 0.0f ),
			_isDown( false )
		...
		void ClickWheel::createPins()
		{
			addInlet( new InletPin( new DataContainer<SampleFrame>(), "in", this ) );

			addOutlet( new OutletPin( new DataContainer<SampleFrame>(), "pos", this ) );
			addOutlet( new OutletPin( new DataContainer<SampleFrame>(), "delta", this ) );
			addOutlet( new OutletPin( new DataContainer<SampleFrame>(), "isDown", this ) );
		}

#ifdef __SUPPORT_GUI
		bool ClickWheel::drawUI()
		{
			if( !Op::drawUI() )
				return false;

			ImGui::SliderFloat( "thresh", &_blobThresh, 0, 1 );

			return true;
		}
#endif

		bool ClickWheel::process()
		{
			SampleFrame *sf = fetchInput<SampleFrame>( "in" );
			...
			if( sf )
			{
				...
				if( sf->height() != 1 || sf->depth() != 1 )
				{
					safeDelete( sf );
					throw std::runtime_error( "clickWheel so far only implemented for 1D frames" );
				}
				...
				//just in case there are multiple blobs, find blob with highest peak
				int maxPos = 0;
				float maxVal = src[0];
				for( int i = 1; i < sf->width(); i++ )
					if( maxVal < src[i] )
					{
						maxVal = src[i];
						maxPos = i;
				...
				float pos = 0.0f;
				if( maxVal > _blobThresh )
				{
					_isDown = true;

					//find beginning of blob
					int blobStartPos = maxPos;
					for( int i = 1; i < sf->width() - 1; i++ )
					{
						int p = ( maxPos - i + sf->width() ) % sf->width();
						if( src[p] < _blobThresh )
						{
							blobStartPos = p;
							break;
						}
					...
					//find end of blob
					int blobEndPos = maxPos;
					for( int i = 1; i < sf->width() - 1; i++ )
					{
						int p = ( blobStartPos + i ) % sf->width();
						if( src[p] < _blobThresh )
						{
							blobEndPos = p;
							break;
					...
					//calculate COM of blob
					float com = 0.0f;
					float sum = 0.0f;

					int blobSize = blobStartPos < blobEndPos ? blobEndPos - blobStartPos : ( blobEndPos + sf->width() - blobStartPos );
					for( int i = blobStartPos; i < blobStartPos + blobSize; i++ )
					{
						com += src[i % sf->width()] * i;
						sum += src[i % sf->width()];
					}
					...
					{
						float intPart = 0;
						pos = modf( com / ( sum * sf->width() ), &intPart );
					}
				}
				else
				{
				...
				float delta = ( wasDown ? _currentPos - pos : 0.0f );
				if( delta > 0.5f )
					delta -= 1.0f;// ( _currentPos - 1.0f ) - pos;
				else if( delta < -0.5f )
					delta += 1.0f;//= ( _currentPos + 1.0f ) - pos;

				_currentPos = pos;

				SampleFrame *sfPos = new SampleFrame( 1, 1, sf->timeStamp(), 1 );
				SampleFrame *sfDelta = new SampleFrame( 1, 1, sf->timeStamp(), 1 );
				SampleFrame *sfIsDown = new SampleFrame( 1, 1, sf->timeStamp(), 1 );

				sfPos->values()[0] = _currentPos;
				sfDelta->values()[0] = delta;
				sfIsDown->values()[0] = _isDown;

				safeDelete( sf );

				drawFrame( sfPos );

				pushOutput( "pos", sfPos );
				pushOutput( "delta", sfDelta );
				pushOutput( "isDown", sfIsDown );
		...
		bool ClickWheel::loadFromJSON( const nlohmann::json &j )
		{
			bool ret = Op::loadFromJSON( j );

			load<float>( j, "blobThresh", _blobThresh );

			return ret;
		}

		bool ClickWheel::saveToJSON( nlohmann::json &j ) const
		{
			bool ret = Op::saveToJSON( j );

			save( j, "blobThresh", _blobThresh );
			return ret;
		}
		*/
	}
