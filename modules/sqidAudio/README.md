# sqıd Plugin for Audio

This repository contains the source code for an audio device plugin (currently audio input only) for the [sqıd visual programming environment](https://github.com/eyeco/sqid). Building on [RtAudio](https://caml.music.mcgill.ca/~gary/rtaudio/), audio devices supported are:
- Windows: ASIO, WASAPI, Direct Sound
- Apple (not tested): Core Audio
- Linux (not tested): ALSA, Pulse Audio, OSS
- UNIX (not tested): Jack Client
